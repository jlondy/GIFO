...
class Search extends Component {
	...
	async handleFavourite(id) {
		...
		await axios
			.post('https://6t35dobcna.execute-api.us-east-1.amazonaws.com/dev/api-lambda-dynamo-gifs', {
					...
					{this.state.search ? (
						<div className="search">
							<form
								...
								style={{ width: '100%', paddingTop: '20px', textAlign: 'center' }}
								autoComplete="off"
							>
								<FormControl style={{ width: '55%' }}>
									<div style={{ paddingBottom: '60px', width: '100%' }}>
										<InputLabel htmlFor="search" style={{ color: '#61dafb' }}>
											Search
										</InputLabel>
										<Input
											onChange={this.handleSearchTag}
											value={this.state.searchTag}
											style={{ width: '100%', color: '#61dafb' }}
											type="text"
											id="searchTag"
										/>
									</div>
									...
									>
										<Card style={{ maxWidth: '365' }}>
											...
											<CardActions disableSpacing style={{ backgroundColor: '#61dafb' }}>
												<IconButton
													onClick={() => this.handleFavourite(gif.id)}
													id={gif.id}
													aria-label="add"
												>
													<FavoriteIcon />
