...
class Account extends Component {
  ...
  async handleDeleteGif(id) {
    ...
    await axios.post(
      'https://6t35dobcna.execute-api.us-east-1.amazonaws.com/dev/api-lambda-dynamo-delete-gif',
      {
  ...
  getGifIds(){
    ...
    axios.post(
      'https://6t35dobcna.execute-api.us-east-1.amazonaws.com/dev/api-lambda-dynamo-get-gifs',
      {
          "email": localStorage.getItem("userEmail")
      })
      .then((res) => {
    ...
    return (
      <div>
        ...
        <div className="account-component">
          <Typography variant="h4" style={{ color: "white", paddingTop: "25px", textAlign: "center"}}>Your favourite Gifs</Typography>
                          ...
                          <Paper key={gif.id} elevation={3} style={{ float: "left", width: "400px", height: "280px", backgroundColor: "#0e151c", padding: "35px"}}>
                              <Card style={{ maxWidth: "365" }}> 
                                  ...
                                  <CardActions disableSpacing style={{backgroundColor: "#61dafb"}}>
                                      <IconButton onClick={() => this.handleDeleteGif(gif.id)} id={gif.id} aria-label="add">
              ...
              (<div className="account">
                  <div className="empty-account">
                      <Paper style={{ backgroundColor: "#0e151c" }} className="account-paper" elevation={24}>
                          <Typography variant="h5" style={{height: "100%", color: "#61dafb", paddingTop: "25px", textAlign: "center"}}>Empty Storage</Typography>
                      </Paper>
                  </div>
              </div>)       
          } 
